...
export class OnConditionBuilder<Entity> {
  ...
  /**
   * Creates a builder whose comparisons resolve columns against the joined relation's own entity
   * metadata, so virtual columns expand into their queries aliased to the relation.
   *
   * @param entityMetadata - metadata of the relation entity the join conditions apply to.
   */
  public static forEntityMetadata<Entity>(entityMetadata: EntityMetadata): OnConditionBuilder<Entity> {
    return new OnConditionBuilder<Entity>(SQLComparisonBuilder.forEntityMetadata<Entity>(entityMetadata))
  }
