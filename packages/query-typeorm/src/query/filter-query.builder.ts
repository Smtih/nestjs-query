...
import { OnConditionBuilder, OnConditionSQL } from './on-condition.builder'
...
export class FilterQueryBuilder<Entity> {
  ...
  public applyRelationJoinsRecursive(
    ...
    return referencedRelations.reduce((rqb, [relationKey, relation]) => {
      ...
      const { condition, params } = this.buildJoinOnCondition(relation)
  ...
  /**
   * Builds the `JOIN ... ON` fragment for a relation's `on` conditions, resolving columns against
   * the joined relation's own entity metadata so its virtual columns expand correctly.
   */
  private buildJoinOnCondition(relation: NestedRelationsAliased[string]): OnConditionSQL {
    return OnConditionBuilder.forEntityMetadata(relation.metadata).build(relation.on, relation.alias)
  }
    ...
    return Object.entries(relations).reduce((prev, [name, relation]) => {
      ...
      return {
        ...
        [name]: {
          ...
          relations: this.injectRelationsAliasRecursive(relationMetadata, relation.children, counter),
