...
describe('FilterQueryBuilder', (): void => {
  ...
  describe('#select', () => {
    ...
    describe('with relation join conditions', () => {
      ...
      it('should expand a virtual column inside the join conditions', () => {
        const selectQueryBuilder = new FilterQueryBuilder(connection.getRepository(TestVirtualColumnEntity)).select({
          filter: { virtualColumnRelations: { on: { siblingCount: { gt: 1 } } } }
        })
        const [sql, params] = selectQueryBuilder.getQueryAndParameters()
        const unquotedSql = sql.replace(/"/g, '')

        expect(unquotedSql).toContain(
          '(SELECT COUNT(*) FROM test_virtual_column_relation WHERE test_virtual_column_entity_id = virtualColumnRelations.test_virtual_column_entity_id) >'
        )
        expect(unquotedSql).not.toContain('virtualColumnRelations.siblingCount')
        expect(formatSql(sql, { params })).toMatchSnapshot()
      })
