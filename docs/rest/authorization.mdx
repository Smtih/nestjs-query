...
The filter is passed to the query service, which evaluates it in JS against the request body before saving. A body that does not match the filter, for example an `ownerId` different from `request.user.id`, fails with `Entity does not meet creation constraints`.

<Warning>
  The filter is evaluated in JS against the flat request body, so filter clauses on relations, or on fields populated by the
  database (such as `createdAt`, generated ids, or column defaults), cannot be checked and will not match.
</Warning>
