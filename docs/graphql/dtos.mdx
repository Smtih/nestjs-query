---
title: 'DTOs'
description: 'The  package leverages most decorators from  and , with the exception of .'
---

## `@FilterableField`[​](#filterablefield 'Direct link to filterablefield')

The `FilterableField` is very similar to the [`Field`](https://typegraphql.ml/docs/types-and-fields.html) from TypeGraphQL, however it allows you to specify the fields that should be filterable when querying.

<Note>If you use the @nestjs/graphql `Field` decorator it will not be exposed in the query type for the DTO.</Note>
...
- `allowedComparisons` - An array of allowed comparisons. You can use this option to allow a subset of filter comparisons when querying through graphql.
  ...
  - It is also the only way to enable the opt-in [`match` full text search comparison](#example---full-text-search-with-match).
...
- `filterOnly`- When set to `true`, the field will only appear as `filter` but isn't included as field inside the `ObjectType`.
  - This option is useful if you want to filter on foreign keys without resolving the relation but you don't want to have the foreign key show up as field in your query type for the DTO. This might be especially useful for [federated relations](/graphql/federation#reference-decorator)
...
### Example - allowedComparisons[​](#example---allowedcomparisons 'Direct link to Example - allowedComparisons')
...
### Example - full text search with match

The `match` comparison performs a database-backed full text search. Unlike the other comparisons it is **never exposed by
default**; a `String` field must explicitly list it in `allowedComparisons` to add it to the schema.
...
export class TodoItemDTO {
  ...
  @FilterableField({ allowedComparisons: ['eq', 'iLike', 'match'] })
  title!: string
}
```
...
```graphql
{
  todoItems(filter: { title: { match: "shopping list" } }) {
    # ...
  }
}
```

The comparison is executed by `@ptc-org/nestjs-query-typeorm` and translates to the database's full text search support.

**MySQL/MariaDB** generates `MATCH (title) AGAINST (:query IN NATURAL LANGUAGE MODE)`, which requires a
[`FULLTEXT` index](https://dev.mysql.com/doc/refman/8.4/en/fulltext-search.html) on the column:
...
**PostgreSQL** generates `title @@ websearch_to_tsquery(:query)`. This is valid on any text column, but to make the search use a
[GIN index](https://www.postgresql.org/docs/current/textsearch-tables.html) declare the filterable column as a stored generated
`tsvector` column and filter on that field instead:
...
export class TodoItemEntity {
  ...
  @Index('todo_item_search_vector_idx', { synchronize: false })
  @Column({
    type: 'tsvector',
...
<Warning>
  `match` is only supported by the TypeORM adapter on PostgreSQL and MySQL/MariaDB. Other databases (e.g. sqlite) and the other
  adapters (sequelize, mongoose, typegoose, mikro-orm), as well as the in-memory filtering used by
  [subscriptions](/graphql/subscriptions), reject the comparison with an error instead of silently returning wrong results.
</Warning>

### Example - filterRequired[​](#example---filterrequired 'Direct link to Example - filterRequired')
...
### Example - filterOnly[​](#example---filteronly 'Direct link to Example - filterOnly')
...
<Note>The `@IDField` uses the same options as `@FilterableField`.</Note>

<Info>
  You may have seen `@IDField` in various examples throughout the docs, this is because we recommend using `@IDField` by default.
...
### Example - Disable Filtering[​](#example---disable-filtering 'Direct link to Example - Disable Filtering')
...
### Setting a default Filter[​](#setting-a-default-filter 'Direct link to Setting a default Filter')
...
<Note>The default filter is only used when a filter is not provided in a query.</Note>
...
### Result Page Size[​](#result-page-size 'Direct link to Result Page Size')
...
### Limiting Results Size[​](#limiting-results-size 'Direct link to Limiting Results Size')
...
<Note>You can disable `maxResultSize` by setting the option to `-1`.</Note>
...
### Paging Strategy[​](#paging-strategy 'Direct link to Paging Strategy')
...
For a more in-depth overview of paging check out the [paging docs](/graphql/queries/paging)
...
### Paging with Total Count[​](#paging-with-total-count 'Direct link to Paging with Total Count')

<Note>This section **ONLY** applies to `CURSOR` and `OFFSET` connections.</Note>

<Warning>
  Enabling `totalCount` can be expensive. If your table is large the `totalCount` query may be expensive, use with caution.
</Warning>

<Info>The `totalCount` field is not eagerly fetched. It will only be executed if the field is queried from the client.</Info>
...
### Default Sort[​](#default-sort 'Direct link to Default Sort')
...
<Info>
  When we use the `@Relation` decorator or other [relation decorators](/graphql/relations) you might want to define the default sorting criteria like this:
...
### Allowed Boolean Expressions[​](#allowed-boolean-expressions 'Direct link to Allowed Boolean Expressions')
...
### Generated filter-type depth[​](#generated-filter-type-depth 'Direct link to Generated filter-type depth')
...
#### `n`-levels deep[​](#n-levels-deep 'Direct link to n-levels-deep')
...
#### infinite depth[​](#infinite-depth 'Direct link to infinite depth')
