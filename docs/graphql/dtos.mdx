...
A relation filter can accept an `on` key whose conditions are injected into that relation's SQL `JOIN ... ON` clause instead of the `WHERE` clause. The key is off by default and is enabled with the `enableRelationJoinConditions` option on the DTO that appears as the relation.
...
`true` uses the default key `on`. Pass `{ field: '<name>' }` instead when the DTO already declares a field called `on`, because the key is the runtime property name as well as the GraphQL field name:
...
The option is resolved from base classes too, so a shared base DTO can enable it once for everything that extends it. Enabling it on a DTO that also declares a filterable field or a relation named the same as the configured key throws at schema generation time.

<Warning>
  Relation join conditions only work for typeorm! The `sequelize`, `mongoose`, `typegoose` and `mikro-orm` adapters
  throw from `forFeature` at application startup when they are asked to serve a DTO that enables the option.
</Warning>
