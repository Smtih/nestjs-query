...
<Info>
  If you are looking to modify incoming requests based on the context, take a look at the [hooks documentation](/graphql/hooks)
</Info>

<Note>Authorization is invoked as the last step before calling the `QueryService`.</Note>
...
## @Authorize Decorator[​](#authorize-decorator 'Direct link to @Authorize Decorator')
...
<Note>
  You can throw an `UnauthorizedException` or return a rejected promise with an `UnauthorizedException` in your authorize
  function, if you can determine at that point that the user should not be able to access the endpoint.
</Note>
...
```ts
import { FilterableField, IDField, FilterableConnection, FilterableRelation, Authorize } from '@ptc-org/nestjs-query-graphql'
...
### Relation Filtering[​](#relation-filtering 'Direct link to Relation Filtering')
...
### Customizing Relation Authorization[​](#customizing-relation-authorization 'Direct link to Customizing Relation Authorization')
...
<Warning>The `auth` option will take precedence over the `@Authorize` decorator on the relation DTO.</Warning>
...
### Custom Authorizer[​](#custom-authorizer 'Direct link to Custom Authorizer')
...
## Using Authorizers In Your Resolver[​](#using-authorizers-in-your-resolver 'Direct link to Using Authorizers In Your Resolver')
...
## @InjectAuthorizer Decorator[​](#injectauthorizer-decorator 'Direct link to @InjectAuthorizer Decorator')
...
<Info>
  If you are extending the `CRUDResolver` directly be sure to [register your DTOs with the
  `NestjsQueryGraphQLModule`](/graphql/resolvers#crudresolver)
</Info>

<Info>
  When using `@InjectAuthorizer`, the injected Authorizer is not the CustomAuthorizer, but the DefaultCRUDAuthorizer that
...
## Authorize depending on operation[​](#authorize-depending-on-operation 'Direct link to Authorize depending on operation')
...
The `operationNames` of the generated CRUD resolver methods are similar to the ones of the [QueryService](/concepts/services):
...
By default the authorizer is invoked for `createOne` and `createMany` (with the `CREATE` operation group), but the filter it returns is not enforced against the input. Unless your `authorize` function throws for create operations, users are able to create records that they would not be allowed to read back.

To enforce the auth filter on create mutations, set the `validateWithAuthFilter` option in the resolver's `create` options. The filter is then passed to the `QueryService`, which evaluates it in JS against each input before saving.
...
With the `TodoItemDTO` from the [getting started](#getting-started) example, where `authorize` returns `{ ownerId: { eq: context.req.user.id } }`, a `createOneTodoItem` mutation with an `ownerId` that does not match the authenticated user is now rejected.

The two mutations behave differently when an input does not match the filter:

- `createOne` throws an error (`Entity does not meet creation constraints`) and nothing is created.
- `createMany` silently skips inputs that do not match the filter and only creates (and returns) the matching subset.

You can also enable the validation for only one of the two mutations:
...
<Warning>
  The auth filter is evaluated in JS against the plain input object, not by the database. This has two limitations:

  - Filters on relations are not supported. Filter clauses that reference relations cannot be checked against the flat input DTO
    and will not match.
  - Filters on fields populated by the database (e.g. `createdAt`, generated ids, or columns with default values) cannot be
    evaluated, because those values do not exist on the input yet.
</Warning>
